window.siteContent = {
  brand: {
    name: 'Mandala Charitable Trust',
    logoAlt: 'Mandala Charitable Trust logo',
    logoSrc: 'img/mandala-logo.jpg',
    ctaLabel: 'Support our work'
  },
  nav: [
    { href: '#about', label: 'About' },
    { href: '#programs', label: 'Programs' },
    { href: '#objects', label: 'Trust Objects' },
    { href: '#trustees', label: 'Trustees' },
    { href: '#transparency', label: 'Transparency' },
    { href: '#connect', label: 'Connect' }
  ],
  hero: {
    eyebrow: 'Registered Charitable Trust',
    title: 'Caring for the children of migrant workers at JRC Palladio.',
    description: 'Mandala Charitable Trust offers safe daycare, early learning, and holistic support for families of construction workers in Sarjapura, Bengaluru. We operate entirely through resident volunteers and donations from our community.',
    meta: [
      { label: 'Founded', value: 'August 2023' },
      { label: 'Trustees', value: '10 residents of JRC Palladio' },
      { label: 'Registration No', value: 'SRJ-4-00284-2023-24' }
    ],
    actions: [
      { href: '#programs', label: 'See our programs', variant: 'primary' },
      { href: '#transparency', label: 'Read our mandate', variant: 'ghost' }
    ],
    imageAlt: 'Children learning and playing at the daycare centre',
    video: {
      src: 'https://www.youtube.com/embed/gTnTXsFPnJY?rel=0&modestbranding=1&si=4E3in9sZBKFMku18',
      title: 'Mandala Charitable Trust daycare walkthrough'
    }
  },
  stats: [
    {
      heading: '9 AM - 4 PM',
      body: 'Daily daycare that keeps children safe while their parents earn a livelihood onsite.'
    },
    {
      heading: '3 Meals Daily',
      body: 'Nutritious breakfast, lunch, and snacks prepared fresh for every child in our centre.'
    },
    {
      heading: '10+ School Enrolments',
      body: 'Children mainstreamed into Handenahalli Government School with books, uniforms, and school kits.'
    }
  ],
  programs: {
    heading: 'What We Do',
    intro: 'Our programmes respond to the immediate needs of migrant families while creating pathways to long-term wellbeing.',
    items: [
      {
        icon: 'img/icon-education.svg',
        iconAlt: 'Daycare icon',
        title: 'Holistic Daycare',
        description: 'Safe, nurturing care for children below six years living at construction sites around JRC Palladio.',
        points: [
          'Structured day from 9 AM to 4 PM with circle time and play-based learning',
          'Nutritious breakfast, lunch, and evening snacks every day',
          'Teachers and caretakers (aayas) hired from the local community'
        ]
      },
      {
        icon: 'img/icon-health.svg',
        iconAlt: 'Education icon',
        title: 'Education Pathways',
        description: 'Bridging children into formal schooling and ensuring they stay enrolled with confidence.',
        points: [
          'Enrolment drives with Government School, Handenahalli',
          'Complete school kits: uniforms, bags, stationery, and seasonal essentials',
          'Learning support sessions for children who miss school during relocations'
        ]
      },
      {
        icon: 'img/icon-livelihood.svg',
        iconAlt: 'Community icon',
        title: 'Community and Culture',
        description: 'Celebrating moments that help families feel welcome and respected within the neighbourhood.',
        points: [
          'Festival gatherings for Diwali, Christmas, and local observances',
          'Distribution of new clothes, gifts, and hygiene kits',
          'Relief support during emergencies or unexpected crises'
        ]
      }
    ]
  },
  objects: {
    heading: 'Objects Of The Trust',
    intro: 'The Mandala Charitable Trust deed is registered under Sections 2(15) and 80G of the Income Tax Act, 1961. Our founding document empowers us to pursue the following objectives:',
    items: [
      'Provide relief to people experiencing poverty through emergency support and basic amenities.',
      'Advance education, including early childhood development, schools, training institutes, and libraries.',
      'Promote health care access and medical relief for vulnerable communities.',
      'Preserve the environment by supporting rainwater harvesting, lake conservation, and green initiatives.',
      'Support cultural, social, and sports programmes that nurture talent and community wellbeing.',
      'Offer financial assistance, scholarships, and rehabilitation for individuals in distress.',
      'Respond to natural calamities with immediate and long-term relief efforts.',
      'Mobilise donations, grants, and endowments to sustain charitable activities without personal benefit to trustees.'
    ]
  },
  trustees: {
    heading: 'Trustees',
    intro: 'Mandala Charitable Trust is stewarded by residents of JRC Palladio who volunteer their time, resources, and expertise.',
    members: [
      'Anandappan C',
      'Anandshekar',
      'Atul Joshi',
      'Debal Mitra',
      'Harish Mohan',
      'Jayanandan P',
      'Haimanti Sarkar',
      'Phani Chinguluri',
      'Senthil Kumar',
      'SREENIVASULU K',
    ]
  },
  transparency: {
    heading: 'Accountability and Transparency',
    intro: 'Our trust deed emphasises good governance, financial discipline, and compliance with the Indian Trusts Act and Income Tax Act.',
    cards: [
      {
        title: 'Governance',
        body: 'The Board of Trustees (five members, elected every two years) manages day-to-day affairs and safeguards the trust fund. Trustees hold office for life unless they resign or are removed for cause.'
      },
      {
        title: 'Compliance',
        body: 'Accounts are maintained transparently and audited annually by a Chartered Accountant. All investments comply with Section 13(1)(d) read with Section 11(5) of the Income Tax Act, 1961.'
      },
      {
        title: 'Ethos',
        body: 'Trust funds are irrevocably dedicated to charitable causes, prohibiting personal gain. Upon dissolution, assets transfer to a like-minded charitable institution.'
      },
      {
        title: 'Regulatory IDs',
        body: '',
        list: [
          'PAN: AAITM8743J',
          'TAN: BLRM45580D',
          'Registration No: SRJ-4-00284-2023-24'
        ]
      }
    ]
  },
  footer: {
    officeHeading: 'Registered Office',
    officeLines: [
      'JRC Palladio Community,',
      'Sarjapura Road, Anekal Taluk,',
      'Bengaluru - 562125'
    ],
    involvedHeading: 'Get Involved',
    involvedText: 'Residents and well-wishers can volunteer, sponsor meals, or donate learning materials. Reach out to any trustee to partner with us.',
    contactHeading: 'Contact',
    contactText: 'Email updates are being set up. In the interim please write to',
    contactPhoneLabel: 'Mobile:',
    contactPhone: '9606025415',
    contactEmail: 'mandala.net.in@gmail.com',
    copyName: 'Mandala Charitable Trust',
    copyLegal: 'Registered under Indian Trusts Act, 1882'
  }
};
